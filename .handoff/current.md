...
- 最新历史快照：[`handoff-20260719-phase00-issues-03-06-done.md`](./handoff-20260719-phase00-issues-03-06-done.md)；
...
- Phase 历史快照：[`phases/phase00/archive/handoff-20260719-phase00-issues-03-06-done.md`](./phases/phase00/archive/handoff-20260719-phase00-issues-03-06-done.md)。
...
- `owner.json` 数据结构落地；
- 原子抢锁、心跳更新、死锁 (Stale Lock) 识别算法；
- 锁冲突时返回 `OPERATION_BUSY` (exit code 10) 与 `busy` 标记；
- 单元测试覆盖锁竞争与清理。

不要先写平台 Adapter，再回头补锁。

## 可并行与后续任务

```text
DS-FND-005 (续) / Issue 08
Transaction Journal 崩溃恢复策略与操作原子回滚。
```

问题文件：[`.scratch/phase-00-foundation/issues/08-transaction-journal-crash-recovery.md`](../.scratch/phase-00-foundation/issues/08-transaction-journal-crash-recovery.md)
...
operationLockAndJournal: in-progress
...
```text
执行 DS-FND-005 / Issue 07：落地 Operation Lock (owner.json) 原子锁与死锁 (Stale Lock) 抢占防线。
```
...
Phase 00 的 DS-QA-001 (Issue 01-02) 和 DS-FND-002 (Issue 03-06: Schema/Envelope, Reference Runner, capabilities, status, listThemes) 已全量测试通过并完成。接下来执行 DS-FND-005 / Issue 07：落地 Operation Lock (owner.json) 原子锁与死锁识别与抢占。测试必须使用隔离 state root。

不要先写平台 Adapter，再回头补锁；不要提前进行 Theme Manager 大规模 UI 或 Desktop Shell 最终选型。
```
