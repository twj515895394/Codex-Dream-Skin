...
- [x] JSON Schema 文件（或等价 TypeScript 类型）覆盖 Request/Response/Error 全部字段
- [x] Schema 校验工具能对合法/非法 JSON 给出明确通过/拒绝
- [x] 退出码常量表和映射函数已实现
- [x] Error code 枚举和 category 映射已实现
- [x] 单元测试验证 Envelope 序列化/反序列化的正确性
- [x] 单元测试验证 request 大小限制、未知字段处理、apiVersion 校验
- [x] 文档与代码中的 Schema 定义保持一致
...
- **完成总结报告路径**：[`.scratch/phase-00-foundation/reports/03-runtime-api-v1-schema-envelope-summary.md`](../reports/03-runtime-api-v1-schema-envelope-summary.md)
- **生成时间**：2026-07-19T02:13:21Z
- **自动化测试**：全量测试套件（含 17 项 Envelope/Schema/ExitCode 校验断言）已 100% PASS。
