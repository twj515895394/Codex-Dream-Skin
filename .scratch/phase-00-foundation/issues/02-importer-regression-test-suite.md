...
- [ ] 至少 15 个独立测试用例，覆盖上述全部类别
- [ ] 测试使用隔离 `STATE_ROOT` 临时目录，不接触真实用户数据
- [ ] 每个安全边界测试验证拒绝发生在正确的阶段（预检 / payload / staging）
- [ ] 失败恢复测试验证失败后文件系统状态干净
- [ ] 测试可通过 `npm test` 或等价命令一键运行
- [ ] 测试在 macOS 上通过；Windows 可选但需标记 skip 原因
